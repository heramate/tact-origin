...
namespace RACTClient
{
    ...
    public class MCSmallTerminal : UserControl
    {
        ...
        // 2015-06-01 - 신윤남 - 컬럼 사이즈 늘리기
        private int m_ColumnsCount = 800;
        ...
        protected override void OnResize(System.EventArgs e)
        {
            ...
            for (int i = 0; i < tVisiblebuffer.Count; i++)
            {
                for (int tColumnIndex = 0; tColumnIndex < tColumns; tColumnIndex++)
                {
                    ...
                    // 2019-01-18 Edit_LMW 수정 - [i]의 값이 m_CharGrid의 index를 초과하는 값으로 넘어오는 문제가 있어 오류가 발생하여 수정함.
                    if (m_CharGrid.Length > i)
            ...
            for (int tColumnIndex = 0; tColumnIndex < tColumns; tColumnIndex++)
            {
                ...
                // 2019-01-18 Edit_LMW 수정 - [i]의 값이 m_CharGrid의 index를 초과하는 값으로 넘어오는 문제가 있어 오류가 발생하여 수정함.
                if (m_CharGrid.Length - 1 > tLastline)
        ...
        protected override void OnMouseMove(MouseEventArgs CurArgs)
        {
            ...
            try
            {
                ...
                if (tEndRow < tBegRow)
                {
                    ...
                    tEndRow = tBegRow;
                    tBegRow = i;
                    for (int tCurRow = tBegRow; tCurRow <= tEndRow - 1; tCurRow++)
                    {
                        if (tCurRow <= 0) continue;

                        for (int tCurCol = 0; tCurCol < this.m_Cols - 1; tCurCol++)
                        {
        ...
        private void ShowNotFindMessage(TelnetStringFindHandlerArgs aArgs)
        {
            ...
            AppGlobal.ShowMessageBox(AppGlobal.s_ClientMainForm, "'" + aArgs.FindString + "'을(를) 찾을 수 없습니다.", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
        ...
        private void mnuPaste_Click(object sender, System.EventArgs e)
        {
            ...
            if (tClipboard.GetDataPresent(DataFormats.Text))
            {
                ...
                {
					// 2019-11-10 개선사항 (다중 명령어 처리 기능 개선 )
                    PastePosition = m_ScrollbackBuffer.Count;
                    IsPaste = true;
        ...
        /*
		 * 2019-11-10 개선사항 (다중 명령어 처리 기능 개선 )
         * 명령어 실행 동작은 엔터 수행시 마지막 라인만 처리.
         * 
         * 명령어 실행 창에 한줄이상의 명령어을 붙여 넣기 할시 
         * 수행 가능 하도록 지원 하기 위한 함수
         */
        private String GetPasteStr()
        {
            String PasteStr = String.Empty;
            int Diff = 0;

            if(IsPaste)
            {
                Diff = m_ScrollbackBuffer.Count - PastePosition;
                if (Diff > 0)
                {
                    for (int i = Diff ; i > 0; i--)
                    {
                        if(i == 1)
                            PasteStr += m_ScrollbackBuffer[m_ScrollbackBuffer.Count - i];
                        else
                            PasteStr += m_ScrollbackBuffer[m_ScrollbackBuffer.Count - i] + "\x0D";
                    }
                }
            }
            return PasteStr;
                ...
                switch (aKeyInfo.ScanCode)
                {
                    ...
                    case 28:
                        ...
                        if (OnSendCommandToTerminalEvent != null)
                        {
                            if(!IsPaste)
                            {
                                OnSendCommandToTerminalEvent(m_SendTerminalList, m_ScrollbackBuffer[m_ScrollbackBuffer.Count - 1] + "\x0D");
                            }else
                            {
                                OnSendCommandToTerminalEvent(m_SendTerminalList, GetPasteStr() + "\x0D");
                                IsPaste = false;
                                PastePosition = 0;
                            }
                        }
