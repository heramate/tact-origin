...
namespace RACTClient
{
    ...
    public class MCTerminalEmulator : SenderControl, ISerialEmulator, ITelnetEmulator
    {
        ...
        // 너무 오래 한 번에 처리하면 UI가 멈추므로 "틱당" 상한을 둡니다.
        private const int RX_FLUSH_INTERVAL_MS = 16;//33;               // 약 30fps
        private const int RX_MAX_FLUSH_CHARS_PER_TICK = 64 * 1024; // 틱당 최대 64KB
        ...
        // ===== Dirty row index list (avoid O(rows) scan per frame) =====
        private System.Collections.Generic.List<int> _dirtyRowList = new System.Collections.Generic.List<int>(256);
        private bool[] _dirtyRowQueued;
        private bool _fullRedraw = true;
        private bool _useDirtyRedraw = true;
        // ===== Cached brushes (avoid per-frame allocations) =====
        private SolidBrush _bgBrushCache;
        private Color _bgBrushColorCache = Color.Empty;

        private SolidBrush GetBgBrush(Color c)
        {
            if (_bgBrushCache == null || _bgBrushColorCache != c)
            {
                if (_bgBrushCache != null) _bgBrushCache.Dispose();
                _bgBrushCache = new SolidBrush(c);
                _bgBrushColorCache = c;
            }
            return _bgBrushCache;
        }
        // ===== Frame buffer (for dirty-row redraw + caret blink) =====
        // Dirty-row 모드에서는 "변경된 줄만" 다시 그리지만,
        // WinForms는 OnPaint 때마다 배경이 지워질 수 있어서(특히 caret blink로 Invalidate가 주기적으로 발생),
        // 이전 프레임을 유지할 수 있는 백버퍼(Bitmap)를 둡니다.
        ...
        private void EnsureFrameBuffer()
        {
            ...
            if (_frameBuffer == null || _frameBuffer.Width != w || _frameBuffer.Height != h)
            {
                ...
                // 백버퍼 렌더링 품질/속도 설정 (원본 OnPaint와 동일하게)
        ...
        //활성화된 터미널을 체크 하기 위한 임시 변수 테스트 20250116
        private bool m_TermialActive = false;
        public bool IsTermialActive
        {
            get { return m_TermialActive; }
            set { m_TermialActive = value; }
        }
...
this.SetStyle(ControlStyles.UserPaint | ControlStyles.AllPaintingInWmPaint | ControlStyles.OptimizedDoubleBuffer | ControlStyles.Opaque | ControlStyles.ResizeRedraw, true);
            ...
            // Rx batching timer (수신 데이터 UI 버벅임 개선)
...
this.SetStyle(ControlStyles.UserPaint | ControlStyles.AllPaintingInWmPaint | ControlStyles.OptimizedDoubleBuffer | ControlStyles.Opaque | ControlStyles.ResizeRedraw, true);
        ...
        protected override void Dispose(bool disposing)
        {
            ...
            if (disposing)
            {
                if (_bgBrushCache != null) { _bgBrushCache.Dispose(); _bgBrushCache = null; }
            }
            // run-render brush cache dispose
            if (_brushCache != null)
            {
                foreach (var kv in _brushCache)
                {
        ...
        protected override void OnResize(System.EventArgs e)
        {
            ...
            //System.Console.Write("OnResize this.m_Caret.Pos.Y = {0} \n", this.m_Caret.Pos.Y);
        ...
        protected override void OnPaint(System.Windows.Forms.PaintEventArgs e)
        {
            ...
            // 2) 내용이 변경된 경우에만 백버퍼를 갱신
            //    - 전체 다시그림(_fullRedraw) 또는
            //    - dirty row가 존재하는 경우
            if (!_frameValid || !_useDirtyRedraw || _fullRedraw || HasAnyDirtyRow())
            {
                ...
                // Redraw()는 _useDirtyRedraw + _dirtyRows 를 보고
                // "전체/부분" 렌더링을 알아서 수행합니다.
        ...
        {
            // 백버퍼를 사용하므로 기본 배경 지우기를 막아 깜빡임/지워짐을 방지
            // (특히 caret blink로 Invalidate가 주기적으로 발생하는 경우)
        }
        ...
        void mnuClear_Click(object sender, EventArgs e)
        {
            ...
            //_frameValid = false;
            //this.Invalidate();
            //System.Console.Write("mnuClear_Click this.m_Caret.Pos.Y = {0} \n", this.m_Caret.Pos.Y);
            CaretToAbs(0, 0);
        ...
        void m_VertScrollBar_OnChangeScrollValue(int aValue)
        {
            ...
            lock (this)
            {
                ...
                if (m_LastVisibleLine == 0)
                {
                    ...
                    //System.Console.Write("m_VertScrollBar_OnChangeScrollValue this.m_Caret.Pos.Y = {0} \n", this.m_Caret.Pos.Y);
        ...
        void m_HorzScrollBar_OnChangeScrollValue(int aValue)
        {
            ...
            lock (this)
            {
                ...
                if (m_LastVisibleLine == 0)
                {
                    ...
                    //System.Console.Write("m_HorzScrollBar_OnChangeScrollValue this.m_Caret.Pos.Y = {0} \n", this.m_Caret.Pos.Y);
                    this.CaretToAbs(this.m_TextAtCursor.Length, this.m_Rows - 1);
                }
                else
                {
       ...
       private void OnReceivedData(string aResult)
        {
            try
            {
                ...
                // 탭 처리만 하고, UI/파싱/리프레시는 배치 타이머에서 처리합니다.
                aResult = aResult.Replace("\t", "    ");

                // 수신 데이터를 큐에 적재 (Invoke 남발 방지)
        ...
        /// <summary>
        /// 수신 데이터를 일정 주기로 묶어서 파싱/리프레시 합니다. (UI 버벅임 개선)
        /// </summary>
        private void RxFlushTimer_Tick(object sender, EventArgs e)
        {
            try
            {
                ...
                // 선택영역 취소(초기화) - 기존 OnReceivedData에서 매번 호출하던 것을 "틱당 1회"로 축소
                //Deselect();

                int totalChars = 0;
                StringBuilder sb = new StringBuilder(4096);

                while (totalChars < RX_MAX_FLUSH_CHARS_PER_TICK && _rxQueue.TryDequeue(out string chunk))
                {
                    sb.Append(chunk);
                ...
                // ? 수신 처리 후 화면 갱신 보장 (입력/스크롤이 없어도 갱신되게)
                //full redraw는 dirty 기반으로 처리합니다. _fullRedraw = true;      // 다음 OnPaint에서 화면 전체 재그리기
            //    _frameValid = false;     // 프레임버퍼 무효화
            //    this.Invalidate();       // 즉시 Paint 요청
            }
            catch { }
        }
        ...
        public void DispatchMessage(Object aSender, string aText)
       {
        ...
        {
            // Use TextRenderer sizing with NoPadding so cell width matches TextRenderer.DrawText.
            // This avoids slow drift between drawn text and caret when doing run rendering.
            ...
            // TextRenderer.MeasureText can return 0 height on some fonts if Size.Empty is used; guard it.
            if (sz.Width <= 0 || sz.Height <= 0)
            {
                // fallback to previous Graphics measurement
                SizeF f = aCurGraphics.MeasureString("A", this.Font);
                sz = new Size((int)(f.Width + 0.5f), (int)(f.Height + 0.5f));
            }
...
// ===== Run Rendering (Putty-style) =====
        private readonly System.Collections.Generic.Dictionary<int, System.Drawing.SolidBrush> _brushCache =
            new System.Collections.Generic.Dictionary<int, System.Drawing.SolidBrush>(64);

        private System.Drawing.Brush GetSolidBrush(System.Drawing.Color c)
        {
            int key = c.ToArgb();
            System.Drawing.SolidBrush b;
            if (_brushCache.TryGetValue(key, out b)) return b;
            b = new System.Drawing.SolidBrush(c);
            _brushCache[key] = b;
            return b;
        }

        // Compare only the fields that affect rendering output for run grouping
        private static bool SameAttribForRun(CharAttribStruct a, CharAttribStruct b)
        {
            if (a.IsBold != b.IsBold) return false;
            if (a.IsBlinking != b.IsBlinking) return false;
            if (a.IsInverse != b.IsInverse) return false;
            if (a.IsUnderscored != b.IsUnderscored) return false;
            if (a.IsDECSG != b.IsDECSG) return false;

            if (a.UseAltColor != b.UseAltColor) return false;
            if (a.UseAltColor && a.AltColor.ToArgb() != b.AltColor.ToArgb()) return false;

            if (a.UseAltBGColor != b.UseAltBGColor) return false;
            if (a.UseAltBGColor && a.AltBGColor.ToArgb() != b.AltBGColor.ToArgb()) return false;

            return true;
        }

        private static bool NeedsCharFallback(CharAttribStruct at)
        {
            // DECSG (line drawing / special) should go through ShowChar -> ShowSpecialChar
            if (at.IsDECSG) return true;
            return false;
        }

        /// <summary>
        /// Draw a row using contiguous runs of identical attributes (putty-style).
        /// Uses existing AssignColors/ShowChar for compatibility with AltColor/AltBGColor/Inverse/Bold/Blink.
        /// </summary>
        private void DrawRowRuns(System.Drawing.Graphics g, int rowY, char[] rowChars, CharAttribStruct[] rowAttribs)
        {
            DrawRowRuns(g, rowY, rowChars, rowAttribs, 0, this.m_Cols - 1);
        }
        private void DrawRowRuns(System.Drawing.Graphics g, int rowY, char[] rowChars, CharAttribStruct[] rowAttribs, int startCol, int endCol)
        {
            if (rowChars == null || rowAttribs == null) return;
            ...
            int cellW = this.m_CharSize.Width;
            int cellH = this.m_CharSize.Height;
            ...
            while (x <= endCol)
            {
                // blank cell: already cleared by row background
                if (rowChars[x] == '\0')
                {
                    x++;
                    continue;
                ...
                CharAttribStruct runAttr = rowAttribs[x];
                ...
                while (runEnd <= endCol)
                {
                    if (rowChars[runEnd] == '\0') break;
                    if (!SameAttribForRun(runAttr, rowAttribs[runEnd])) break;
                    runEnd++;
                }

                int runLen = runEnd - runStart;
                int xPix = runStart * cellW;

                System.Drawing.Color fg = System.Drawing.Color.White;
                System.Drawing.Color bg = System.Drawing.Color.Black;
                AssignColors(runAttr, ref fg, ref bg);

                // If background differs from control BackColor, paint it for the run region
                if (bg.ToArgb() != this.BackColor.ToArgb())
                {
                    g.FillRectangle(GetSolidBrush(bg), xPix, yPix, runLen * cellW, cellH);
                }

                // Fallback for special drawing (DECSG etc.)
                if (NeedsCharFallback(runAttr))
                {
                    for (int i = 0; i < runLen; i++)
                    {
                        char ch = rowChars[runStart + i];
                        if (ch == '\0') continue;

                        int cx = (runStart + i) * cellW;
                        // Use ShowChar signature: (Graphics, Char, xPx, yPx, attrib)
                        this.ShowChar(g, ch, cx, yPix, runAttr);
                    }
                ...
                // Run string draw (single call)
                string s = new string(rowChars, runStart, runLen);

                // Use TextRenderer to keep fixed cell alignment (prevents caret drifting vs text)
                ...
                // Underline: one line for the whole run
                if (runAttr.IsUnderscored)
                {
                    using (var p = new System.Drawing.Pen(fg, 1))
                    {
                        int ulY = yPix + m_UnderlinePos;
                        g.DrawLine(p, xPix, ulY, xPix + runLen * cellW, ulY);
                    }
                }
        ...
        private void ShowChar(Graphics aCurGraphics, Char aCurChar, Int32 aX, Int32 aY, CharAttribStruct aCurAttribs)
        {
                ...
                aCurChar == '`'))
            {
	      	...
	      	// Use TextRenderer for normal characters to match run rendering metrics (prevents caret/text misalignment)
			TextRenderer.DrawText(
			    aCurGraphics,
			    aCurChar.ToString(),
			    this.Font,
			    new Rectangle(aX, aY, this.m_CharSize.Width, this.m_CharSize.Height),
			    tCurFGColor,
                tCurBGColor,
                TextFormatFlags.NoPadding | TextFormatFlags.NoPrefix | TextFormatFlags.SingleLine
			);
        ...
        private void ClearDown(Int32 aParam)
        {
            ...
            switch (aParam)
            {
                case 0: // from cursor to bottom inclusive
                    ...
                    //MarkDirtyRange(tY, this.m_Rows - 1);
                    break;
            ...
            if (_useDirtyRedraw)
            {
                // ClearDown affects multiple rows depending on parameter.
                switch (aParam)
                {
                    case 0:
                        // current line: from cursor to end
                        MarkDirtyXRange(tY, tX, this.m_Cols - 1);
                        // lines below: full rows
                        if (tY + 1 <= m_BottomMargin) MarkDirtyRange(tY + 1, m_BottomMargin);
                        break;
                    case 1:
                        // lines above: full rows
                        if (tY - 1 >= 0) MarkDirtyRange(0, tY - 1);
                        // current line: from start to cursor
                        MarkDirtyXRange(tY, 0, tX);
                        break;
                    case 2:
                        MarkDirtyRange(0, m_BottomMargin);
                        break;
                    default:
                        MarkDirtyRow(tY);
                        break;
                ...
                // Invalidate only affected region(s)
                switch (aParam)
                {
                    case 0:
                        InvalidateSpan(tY, tX, this.m_Cols - 1);
                        if (tY + 1 <= m_BottomMargin) InvalidateRows(tY + 1, m_BottomMargin);
                        break;
                    case 1:
                        if (tY - 1 >= 0) InvalidateRows(0, tY - 1);
                        InvalidateSpan(tY, 0, tX);
                        break;
                    case 2:
                        InvalidateRows(0, m_BottomMargin);
                        break;
                    default:
                        InvalidateRow(tY);
                        break;
        ...
        private void ClearRight(Int32 aParam)
        {
            ...
            switch (aParam)
            {
                case 0: // from cursor to end of line inclusive
                    Array.Clear(this.m_CharGrid[tY], tX, this.m_CharGrid[tY].Length - tX);
                    Array.Clear(this.m_AttribGrid[tY], tX, this.m_AttribGrid[tY].Length - tX);
                     if (_useDirtyRedraw) { MarkDirtyXRange(tY, tX, this.m_Cols - 1); InvalidateSpan(tY, tX, this.m_Cols - 1); }
                    break;

                case 1: // from beginning to cursor inclusive
                    Array.Clear(this.m_CharGrid[tY], 0, tX + 1);
                    Array.Clear(this.m_AttribGrid[tY], 0, tX + 1);
                    if (_useDirtyRedraw) { MarkDirtyXRange(tY, 0, tX); InvalidateSpan(tY, 0, tX); }
                    break;
                ...
                case 2: // entire line
                    ...
                    if (_useDirtyRedraw) { MarkDirtyRow(tY); InvalidateRow(tY); }
        ...
        {
            // 파서가 m_CharGrid/m_AttribGrid를 갱신한 뒤 호출
            // 여기서는 "화면이 바뀐 줄"을 dirty 로 표시하고 프레임버퍼를 무효화한 뒤 다시 그리게 합니다.
            //if (_useDirtyRedraw)
           // {
                // (정교한 dirty 추적은 PutChar/Clear/Scroll 등에서 MarkDirtyRow/Range로 처리)
                // 안전하게 전체를 dirty 처리하되, WipeScreen(전체 지우기) 없이 백버퍼 위에 다시 그리게 합니다.
                //MarkAllDirty();
            //}
            //else
            //{
        ...
        private void Redraw(System.Drawing.Graphics tCurGraphics)
        {
            ...
            // refresh the screen (dirty rows only)
// refresh the screen
            if (_useDirtyRedraw && !_fullRedraw && _dirtyRowList != null && _dirtyRowList.Count > 0)
            {
                int[] rows = _dirtyRowList.ToArray();
                _dirtyRowList.Clear();

                foreach (int Y in rows)
                {
                    if (Y < 0 || Y >= m_Rows) continue;
                    if (_dirtyRows != null && Y < _dirtyRows.Length && !_dirtyRows[Y]) continue;

                    int x1, x2;
                    if (!TryGetDirtyXRange(Y, out x1, out x2)) { x1 = 0; x2 = m_Cols - 1; }

                    int rowYpx = Y * m_CharSize.Height;
                    int cellW = m_CharSize.Width;
                    int cellH = m_CharSize.Height;

                    int clearX = x1 * cellW;
                    int clearW = (x2 - x1 + 1) * cellW;

                    tCurGraphics.FillRectangle(GetBgBrush(this.BackColor), clearX, rowYpx, clearW, cellH);
                    DrawRowRuns(tCurGraphics, Y, this.m_CharGrid[Y], this.m_AttribGrid[Y], x1, x2);
            ...
            {
                // full redraw (or fallback)
                for (Int32 Y = 0; Y < m_Rows; Y++)
                {
                    if (_useDirtyRedraw && !_fullRedraw &&
                        _dirtyRows != null && Y < _dirtyRows.Length && !_dirtyRows[Y])
                    {
                        continue;
                    }

                    int x1, x2;
                    if (!(_useDirtyRedraw && !_fullRedraw) || !TryGetDirtyXRange(Y, out x1, out x2))
                    {
                        x1 = 0; x2 = m_Cols - 1;
                    }
        ...
        private void CarriageReturn()
        {
            ...
            if(!tReturn)
                m_IsCheckPrompt = false;
            //System.Console.Write("CarriageReturn this.m_Caret.Pos.Y = {0} \n", this.m_Caret.Pos.Y);
        ...
        private void Tab()
        {
            for (Int32 i = 0; i < m_TabStops.Columns.Length; i++)
            {
                if (i > m_Caret.Pos.X && m_TabStops.Columns[i] == true)
                {
                    ...
                    //System.Console.Write("Tab1 this.m_Caret.Pos.Y = {0} \n", this.m_Caret.Pos.Y);
                    return;
                }
            }

            this.CaretToAbs(this.m_Cols - 1, this.m_Caret.Pos.Y);
            //System.Console.Write("Tab2 this.m_Caret.Pos.Y = {0} \n", this.m_Caret.Pos.Y);
            return;
        }
        ...
        private void ShowCaret(Graphics aCurGraphics)
        {
            ...
            // Guard (avoid out-of-range)
            if (tY < 0 || tY >= m_Rows) return;
            if (tX < 0 || tX >= m_Cols) return;

            int cellX = tX * (int)this.m_CharSize.Width;
            int cellY = tY * (int)this.m_CharSize.Height;

            aCurGraphics.DrawImageUnscaled(this.m_Caret.Bitmap, cellX, cellY);
            ...
            // IMPORTANT: ShowChar expects cell origin (it applies m_DrawstringOffset internally)
            this.ShowChar(aCurGraphics, this.m_CharGrid[tY][tX], cellX, cellY, tCurAttribs);
        ...
        private void CaretUp()
        {
            ...
            //System.Console.Write("CaretUp this.m_Caret.Pos.Y = {0} \n", this.m_Caret.Pos.Y);
        }
        ...
        private void CaretDown()
        {
           ...
           //System.Console.Write("CaretDown this.m_Caret.Pos.Y = {0} \n", this.m_Caret.Pos.Y);
        ...
        private void CaretLeft()
        {
            ...
            //System.Console.Write("CaretLeft this.m_Caret.Pos.Y = {0} \n", this.m_Caret.Pos.Y);
        }

        /// <summary>
        /// BS(Backspace) 처리: 커서를 왼쪽으로 이동시키고 해당 셀을 비워(erase) "잔상"을 방지합니다.
        /// 많은 장비가 BS만 보내는 경우가 있어, 화면 버퍼도 함께 정리합니다.
        /// </summary>
        private void BackspaceErase()
        {
           int y = this.m_Caret.Pos.Y;
            int x = this.m_Caret.Pos.X;

            if (x <= 0) return;

            x--;
            this.m_Caret.Pos.X = x;

            if (y < 0 || y >= this.m_Rows) return;
            ...
            // 해당 행을 다시 그리게 해서 이전 글자가 남는 현상(ghosting) 제거
            if (_useDirtyRedraw) MarkDirtyXRange(y, x, x);
            // 즉시 반영
            //_frameValid = false;
            InvalidateSpan(y, x, x);
        ...
        private void CaretToRel(System.Int32 aY, System.Int32 aX)
        {
            ...
            //System.Console.Write("CaretToRel this.m_Caret.Pos.Y = {0} \n", this.m_Caret.Pos.Y);
        }
        ...
        private void CaretToAbs(Int32 aX, Int32 aY)
        {
            ...
            //System.Console.Write("CaretToAbs this.m_Caret.Pos.Y = {0} \n", this.m_Caret.Pos.Y);
        ...
        private void CommandRouter(object Sender, ParserEventArgs e)
        {
            ...
            switch (e.CurSequence)
            {
                ...
                case "\x1b#8": //DECALN
                    ...
                    {
                        //System.Console.Write("CommandRouter1 this.m_Caret.Pos.Y = {0} \n", this.m_Caret.Pos.Y);
                ...
                case "\x1b[B": // CUD
                    ...
                    //System.Console.Write("CommandRouter2 this.m_Caret.Pos.Y = {0} \n", this.m_Caret.Pos.Y);
                ...
                case "\x1b[A": // CUU
                    ...
                    if (tIncrement == 0) tIncrement = 1;
                    //System.Console.Write("CommandRouter3 this.m_Caret.Pos.Y = {0} \n", this.m_Caret.Pos.Y);
                    this.CaretToAbs(this.m_Caret.Pos.X, this.m_Caret.Pos.Y - tIncrement);
                    break;
                ...
                case "\x1b[C": // CUF
                    ...
                    if (tIncrement == 0) tIncrement = 1;
                    //System.Console.Write("CommandRouter4 this.m_Caret.Pos.Y = {0} \n", this.m_Caret.Pos.Y);
                ...
                case "\x1b[D": // CUB
                    ...
                    if (tIncrement == 0) tIncrement = 1;
                    //System.Console.Write("CommandRouter5 this.m_Caret.Pos.Y = {0} \n", this.m_Caret.Pos.Y);
                    this.CaretToAbs(this.m_Caret.Pos.X - tIncrement, this.m_Caret.Pos.Y);
                    break;
            ...
            foreach (string CurOption in CurParams.Elements)
            {
                ...
                switch (OptInt)
                {
                    ...
                    case 6: // Origin Mode Absolute
                        this.m_Modes.Flags = this.m_Modes.Flags & ~Mode.s_OriginRelative;
                        //System.Console.Write("SetqmlMode this.m_Caret.Pos.Y = {0} \n", this.m_Caret.Pos.Y);
                        this.CaretToAbs(0, 0);
                        break;
        ...
        private void ExecuteChar(Char aCurrentChar)
        {
            switch (aCurrentChar)
            {
                ...
                case '\x08': // BS back space
                    this.BackspaceErase();//this.CaretLeft();
                    break;
                ...
                case '\x85': // NEL Next line
                    this.LineFeed();
                    //System.Console.Write("ExecuteChar this.m_Caret.Pos.Y = {0} \n", this.m_Caret.Pos.Y);
        ...
        private void SetSize(Int32 aRows, Int32 aColumns)
        {
            ...
            */
            ...
            // Dirty-row redraw 초기화 (변경된 줄만 다시 그리기)
            _dirtyRows = new bool[aRows];
            _dirtyMinX = new int[aRows];
            _dirtyMaxX = new int[aRows];
            _dirtyRowQueued = new bool[aRows];
            if (_dirtyRowList != null) _dirtyRowList.Clear();
            for (int i = 0; i < aRows; i++) { _dirtyMinX[i] = aColumns; _dirtyMaxX[i] = -1; }
            _fullRedraw = true;
            _frameValid = false;
            MarkAllDirty();
        }

        private bool HasAnyDirtyRow()
        {
            if (_fullRedraw) return true;
            if (_dirtyRowList != null && _dirtyRowList.Count > 0) return true;
            if (_dirtyRows == null) return false;
            for (int i = 0; i < _dirtyRows.Length; i++)
            {
                if (_dirtyRows[i]) return true;
            }
            return false;
        }


        private void ClearDirtyRow(int y)
        {
            if (_dirtyRows == null) return;
            if (y < 0 || y >= _dirtyRows.Length) return;

            _dirtyRows[y] = false;

            if (_dirtyRowQueued != null && y < _dirtyRowQueued.Length)
                _dirtyRowQueued[y] = false;
            if (_dirtyMinX != null && _dirtyMaxX != null &&
                y < _dirtyMinX.Length && y < _dirtyMaxX.Length)
            {
                _dirtyMinX[y] = m_Cols;   // 초기값: min = cols
                _dirtyMaxX[y] = -1;       // 초기값: max = -1
            }
        }

        
        // ===== Dirty Invalidate Helpers (v2.3) =====
        private void InvalidateSpan(int y, int x1, int x2)
        {
            if (y < 0 || y >= m_Rows) { this.Invalidate(); return; }
            if (x1 > x2) { int t = x1; x1 = x2; x2 = t; }
            x1 = Math.Max(0, x1);
            x2 = Math.Min(m_Cols - 1, x2);

            int cellW = (int)this.m_CharSize.Width;
            int cellH = (int)this.m_CharSize.Height;
            ...
            this.Invalidate(new Rectangle(px, py, w, h));
        }

        private void InvalidateRow(int y)
        {
            InvalidateSpan(y, 0, m_Cols - 1);
        }

        private void InvalidateRows(int y1, int y2)
        {
            if (y1 > y2) { int t = y1; y1 = y2; y2 = t; }
            y1 = Math.Max(0, y1);
            y2 = Math.Min(m_Rows - 1, y2);
            if (y1 > y2) return;
        ...
        private void MarkDirtyXRange(int y, int x1, int x2)
        {
            if (_dirtyRows == null) return;
            if (y < 0 || y >= _dirtyRows.Length) return;
            if (_dirtyMinX == null || _dirtyMaxX == null) { MarkDirtyRow(y); return; }
            ...
            if (_dirtyRowQueued != null && y < _dirtyRowQueued.Length && !_dirtyRowQueued[y])
            {
        ...
        private bool TryGetDirtyXRange(int y, out int x1, out int x2)
        {
            x1 = 0; x2 = this.m_Cols - 1;

            if (_dirtyRows == null || y < 0 || y >= _dirtyRows.Length) return false;
            if (!_dirtyRows[y]) return false;

            if (_fullRedraw) { x1 = 0; x2 = this.m_Cols - 1; return true; }

            if (_dirtyMinX == null || _dirtyMaxX == null) { x1 = 0; x2 = this.m_Cols - 1; return true; }

            int min = _dirtyMinX[y];
            int max = _dirtyMaxX[y];

            // if span empty, fall back to full row to be safe
            if (max < 0 || min > max) { x1 = 0; x2 = this.m_Cols - 1; return true; }
        ...
        private void MarkDirtyRow(int y)
        {
            if (_dirtyRows == null) return;
            if (y < 0 || y >= _dirtyRows.Length) return;
		    ...
		    // Rect 기반이면, row 전체로 span 설정	
            if (_dirtyMinX != null && _dirtyMaxX != null &&
                y < _dirtyMinX.Length && y < _dirtyMaxX.Length)
            {
                _dirtyMinX[y] = 0;
                _dirtyMaxX[y] = m_Cols - 1;
            }
        }

        private void MarkDirtyRange(int y1, int y2)
        {
            if (_dirtyRows == null) return;
            if (y1 > y2) { int t = y1; y1 = y2; y2 = t; }
            y1 = Math.Max(0, y1);
            ...
            for (int y = y1; y <= y2; y++)
                MarkDirtyRow(y); // row 전체로 잡는 게 안전
        }
        ...
        {
            // IMPORTANT:
            // We draw most text via TextRenderer (GDI). Measure cell size with the same system (GDI),
            // otherwise cellW differs and the caret will drift by columns.
            using (Graphics tmpGraphics = this.CreateGraphics())
            {
                // With TextRenderer + NoPadding, we don't need DrawString offset compensation.
                this.m_DrawstringOffset = new Point(0, 0);

                var flags = TextFormatFlags.NoPadding | TextFormatFlags.NoPrefix | TextFormatFlags.SingleLine;
                // Use a representative wide glyph. With a monospaced font, any glyph works, but W is safest.
                Size sz = TextRenderer.MeasureText(tmpGraphics, "W", this.Font, new Size(int.MaxValue, int.MaxValue), flags);

                this.m_CharSize.Width = sz.Width;
                this.m_CharSize.Height = sz.Height;
            }
        ...
        */
        ...
        {
            // Caret blink: only invalidate the caret cell (reduces full-frame repaints)
            m_IsShowCaret = !m_IsShowCaret;

            if (_useDirtyRedraw)
            {
                int cx = this.m_Caret.Pos.X;
                int cy = this.m_Caret.Pos.Y;
                MarkDirtyXRange(cy, cx, cx);
                InvalidateSpan(cy, cx, cx);
            }
            else
            {
                this.Invalidate();
