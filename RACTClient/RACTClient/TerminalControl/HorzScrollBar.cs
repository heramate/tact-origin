...
namespace RACTClient
{
    ...
    public delegate void ChangeHorzScrollValue(int aValue);

    /// <summary>
    /// 스크롤바 입니다.
    /// </summary>
    public class HorzScrollBar : HScrollBar
    {
        public event ChangeScrollValue OnChangeHorzScrollValue;
        ...
        protected int valMax = 800;
        private int m_OldValue = 0;
        private int m_NewValue = 0;

        public HorzScrollBar()
        {
            this.SetStyle(ControlStyles.Selectable, false);
            this.Maximum = 0;
            this.Minimum = -1;
            
            //this.Scroll += new ScrollEventHandler(HorzScrollBar_Scroll);
            //this.ValueChanged += new EventHandler(HorzScrollBar_ValueChanged);
        }

        void HorzScrollBar_ValueChanged(object sender, EventArgs e)
        {
            ...
            if (OnChangeHorzScrollValue != null)
            {
                OnChangeHorzScrollValue(tChangeValue);
            }
            m_OldValue = Value;
            if (Value == Maximum)
            {
        ...
        void HorzcrollBar_Scroll(object sender, ScrollEventArgs e)
        {
            if (!this.Visible) return;
