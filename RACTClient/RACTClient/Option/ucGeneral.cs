...
namespace RACTClient
{
    public partial class ucGeneral : UserControl, IOptionPanal
    {
        ...
        {
			// 2019-11-10 개선사항 (로그 저장 경로 개선)
            folderBrowserDialog1.SelectedPath = GetInitialFolderPath(txtLogPath.Text, AppGlobal.s_ClientOption.LogPath);
        ...
        {
            folderBrowserDialog1.SelectedPath = GetInitialFolderPath(txtScriptPath.Text, AppGlobal.s_ClientOption.ScriptSavePath);
            if (folderBrowserDialog1.ShowDialog() == DialogResult.OK)
            {
        ...
        private string GetInitialFolderPath(string currentPath, string fallbackPath)
        {
            string tPath = currentPath.Trim();
            if (tPath.Length == 0)
            {
                tPath = fallbackPath;
            }
