    ...
    public partial class ucDeviceSearch : SenderControl
    {
        ...
        public void DisplayList(DeviceInfoCollection aDeviceList)
        {
            ...
            try
            {
                ...
                fgDeviceList.Rows.Count = 1;
                // 20260211 ShinMyungsu User 장비접근권한 리턴 데이터가 없으면 그리드에 로를 추가하지 않게 수정, 기존코딩 주석처리
                //fgDeviceList.Rows.Count = aDeviceList.Count + 1;

                foreach (DeviceInfo tDeviceInfo in aDeviceList)
                {
                    // 20260211 ShinMyungsu User 장비접근권한 리턴 데이터가 없으면 그리드에 로를 추가하지 않게 수정, CenterCode 와 IPAddress가 없으면 스킵
                    if (tDeviceInfo.CenterCode.Equals(string.Empty) && tDeviceInfo.IPAddress.Equals(string.Empty)) continue;
                    fgDeviceList.Rows.Count = tRowIndex + 1;
                    //==============================================================================================================
                ...
                // 20260211 ShinMyungsu 장비대수를 aDeviceList.Count -> (tRowIndex - 1)로 수정, aDeviceList에 그리드에 뿌려지지 않는 자료도 있어서
                lblSelectDeviceCount.Text = "조회 장비대수 : " + (tRowIndex - 1).ToString();
