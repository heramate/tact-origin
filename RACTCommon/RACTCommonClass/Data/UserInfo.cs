...
namespace RACTCommonClass
{
    ...
    public class UserInfo : ICloneableEx<UserInfo>
    {
        ...
        private void CopyTo(UserInfo aSource, UserInfo aDest, bool aIsCompactClone)
        {
            ...
            // 20260209 ShinMyungsu 사용자별 장비접근권한
            aDest.m_MangTypes = (ArrayList)aSource.MangTypes.Clone();
        }
        ...
        /// <summary>
        /// 20260209 ShinMyungsu 사용자별 장비접근권한
        /// 사용자의 MangType을 가져오거나 설정합니다.
        /// </summary>
        private ArrayList m_MangTypes = new ArrayList();

        public ArrayList MangTypes
        {
            get { return m_MangTypes; }
            set { m_MangTypes = value; }
        }
        ...
        /// <summary>
        /// 20260209 ShinMyungsu 사용자별 장비접근권한
        /// MangType(망구분)을 DB에 사용할 수 있는 목록으로 가져오기 합니다.
        /// </summary>
        public string GetMangType
        {
            ...
            get
            {
                string tMangTypeList = "";

                for (int i = 0; i < m_MangTypes.Count; i++)
                {
                    tMangTypeList += string.Concat(m_MangTypes[i].ToString(), "''", ",", "''");
                }

                if (tMangTypeList.Length > 0)
                {
                    tMangTypeList = tMangTypeList.Substring(0, tMangTypeList.Length - 5);
                }

                return tMangTypeList;
            }
        }
